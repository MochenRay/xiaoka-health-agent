...
- 只能把以下内容写成已可用：本地文件驱动 Agent、核心记录工作流、稳定路径合同、
  OpenClaw 五条自动化任务、Phase 2B/2C/3A synthetic fixtures 与 validators。
- 必须明确区分 `runtime enabled`、`runtime smoke passed`、`static fixture
  validation` 和 `pending`。
- Phase 2C 截图能力只能声称 synthetic mapping 已验证；真实 screenshot/OCR
...
- 不得声称 Apple Health 历史同步、Health Auto Export、Apple Health XML parser、
...
```bash
git grep -nE '(sk-[A-Za-z0-9_-]{20,}|ghp_[A-Za-z0-9_]{20,}|github_pat_[A-Za-z0-9_]{20,}|AKIA[0-9A-Z]{16}|xox[baprs]-[A-Za-z0-9-]{10,}|AIza[0-9A-Za-z_-]{35})' -- .
```

发布前确认：

- `.env.example` 只有占位符，不含真实 token、API key、chat id 或 webhook。
- `.env`、`*.token`、个人 `config/profile.md`、`config/goals.md` 和
